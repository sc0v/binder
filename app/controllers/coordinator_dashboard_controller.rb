class CoordinatorDashboardController < ApplicationController
  ...
  def search
    ...
    if search_query.present?
      if ["add", "+", "add_to_cart"].include?(search_query)
        perform_add_to_cart
        return redirect_to coordinator_dashboard_path
      elsif ["clear", "clear_cart"].include?(search_query)
        perform_clear_cart
        return redirect_to coordinator_dashboard_path
      elsif ["out", "checkout", "check_out"].include?(search_query)
        return redirect_to coordinator_dashboard_confirm_path("checkout")
      elsif ["in", "checkin", "check_in"].include?(search_query)
        return redirect_to coordinator_dashboard_confirm_path("checkin")
    ...
    redirect_to coordinator_dashboard_path
  end
  ...
  def confirm_post
    ...
    case session[:step]
      ...
      when 4
        ...
        redirect_to coordinator_dashboard_confirm_path
    end
  end
  ...
  def process_confirm_two
    unless params[:organization_id].present?
      ...
      return redirect_to coordinator_dashboard_confirm_path
    end
