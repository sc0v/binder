...
export default class extends Controller {
  ...
  navigate(event) {
    ...
    switch (event.key) {
      case "ArrowDown":
        event.preventDefault()
        this.setActiveIndex((this.activeIndex + 1) % items.length, items)
        break
      case "ArrowUp":
        event.preventDefault()
        this.setActiveIndex((this.activeIndex - 1 + items.length) % items.length, items)
        break
      case "Enter":
