...
module Dashboard
  ...
  class ShowContextBuilder
    ...
    def call
      ...
      checked_out_lift_ids =
        ScissorLiftCheckout.where(checked_in_at: nil).select(:scissor_lift_id)

      {
        ...
        step: @flow&.dig('step'),
        initial_step:
          flow_kind.present? ? FlowDefinition.initial_step(flow_kind) : nil,
        terminal_step:
          @flow.present? ? FlowDefinition.terminal_step?(@flow) : false,
        flow_tools:
          Tool.where(id: Array(@flow&.dig('tool_ids')).map(&:to_i)).index_by(
            &:id
          ),
        flow_participant: flow_participant,
        flow_participant_organizations:
          flow_participant&.organizations&.order(:name).to_a,
        flow_organization:
          Organization.find_by(id: @flow&.dig('organization_id')),
        all_organizations: Organization.order(:name).to_a,
        queued_organizations:
          queued_organizations_for(@flow&.dig('queue_type')),
        queue_current_resource: queue_resource_for(flow_kind),
        flow_lift: ScissorLift.find_by(id: @flow&.dig('scissor_lift_id')),
        checked_out_lifts:
          ScissorLift.where(id: checked_out_lift_ids).order(:name).to_a,
        available_lifts:
          ScissorLift.where.not(id: checked_out_lift_ids).order(:name).to_a,
        lift_overview_resource:
          (Dashboard::ScissorLiftOverviewResource.new if flow_kind == 'lift'),
        lookup_resource: lookup_resource_for(@flow),
        flow_params: FlowState.to_params(@flow)
      }.tap do |context|
        context[:random_available_lift] = context[:available_lifts].sample
      end
    end
    ...
    def lookup_resource_for(flow)
      ...
      when 'tool'
        Tool.find_by(id: flow['lookup_id'])
      when 'participant'
        Participant.find_by(id: flow['lookup_id'])
      when 'organization'
        Organization.find_by(id: flow['lookup_id'])
      when 'scissor_lift'
        ScissorLift.find_by(id: flow['lookup_id'])
      end
    end
