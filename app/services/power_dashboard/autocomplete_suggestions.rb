  ...
  class AutocompleteSuggestions
    ...
    class << self
      def cached_action_suggestions
        @cached_action_suggestions ||=
          Dashboard::ActionRegistry.suggestions.freeze
      end
    end
    ...
    def action_matches?(suggestion)
      ...
      label.start_with?(@normalized) || value.start_with?(@normalized) ||
    ...
    def organization_suggestions
      Organization
        ...
        .map do |org|
          label =
            (
              if org.short_name.present?
                "Organization: #{org.name} (#{org.short_name})"
              else
          ...
          {
            label: label,
            value: org.short_name.presence || org.name,
            type: 'organization'
          }
        end
    end
